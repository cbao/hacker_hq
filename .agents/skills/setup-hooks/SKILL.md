---
name: setup-hooks
description: Configure Codex hooks to report agent activity to the Hacker HQ dashboard. Adds global hooks to ~/.Codex/settings.json so every Codex session auto-reports to the relay server.
user-invocable: true
...
Install Hacker HQ dashboard reporting hooks into the user's **global** Codex settings (`~/.Codex/settings.json`). Once installed, every Codex session on this machine will report tool use, stops, subagent lifecycle, and session boundaries to the relay server at `http://localhost:3001/hook`.

## What this skill does

1. Reads `~/.Codex/settings.json` (creates it with `{}` if missing).
...
5. Writes the merged JSON back to `~/.Codex/settings.json`.
...
python3 - <<'PY'
import json, os, sys
p = os.path.expanduser("~/.Codex/settings.json")
with open(p) as f:
    data = json.load(f)
...
> Hacker HQ hooks installed in `~/.Codex/settings.json`. Every Codex session on this machine will now report activity to `http://localhost:3001/hook`. Start the relay server with `npx tsx server.ts` from the hacker_hq project to see live events.
...
print(f"Removed {removed} Hacker HQ hook entries from ~/.Codex/settings.json")
PY
```
...
- This skill targets **global** settings at `~/.Codex/settings.json`. Do not touch project-level `.Codex/settings.json` or `settings.local.json`.
